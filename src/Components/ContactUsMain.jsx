 ...
 * ContactUsMain.jsx
 * - GSAP removed
 * - Contact form removed (replaced by Quick Contact tiles)
 * - Added clear spacing between Hero -> Map/QuickContact -> Contact Cards
 * - Responsive and keeps original visual style
 */

const ContactUsMain = ({
  bannerImage = "/Hero/DSC08789.JPG",
  bannerTitle,
  bannerSubtitle,
}) => {
  const { t } = useTranslation();
  ...
  // quick actions
  const callPrimary = () => {
    window.location.href = `tel:${phones[0].number.replace(/\s+/g, "")}`;
  };
  const waLink = `https://wa.me/${phones[2].number.replace(
    /\D/g,
    ""
  )}?text=${encodeURIComponent(
    "Hello Shri Samarth Hospital — I would like to consult / book an appointment."
  )}`;
  const displayPhone = phones[2].number; // the reception number

  return (
    <main className="bg-white overflow-x-hidden">
      {/* 1) Full-width Hero */}
      <section className="relative w-full mb-10 sm:mb-14 lg:mb-16">
        <div className="relative w-full">
          <img
            src={bannerImage}
            alt="Hospital reception"
          ...
          <div className="absolute inset-0 flex items-center z-10">
            <div className="px-4 sm:px-10 lg:px-20 w-full">
              <div className="max-w-2xl rounded-2xl bg-white/80 backdrop-blur-md p-6 sm:p-8 ring-1 ring-slate-200 shadow-md">
                <div className="flex items-center gap-2 text-xs font-medium text-slate-700">
                  <Ambulance size={18} className="text-red-500" />
                  {t("contact.emergencySupport")}
                </div>
      ...
      {/* 2) Quick Contact (left) + Map (right) */}
      <section className="relative">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-10 sm:pb-14">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-start">
            {/* LEFT: Quick Contact / Appointments (replaces form) */}
            <div className="rounded-3xl bg-white p-6 sm:p-8 ring-1 ring-slate-200 shadow-sm">
              <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight text-slate-900">
                {t("contact.quickContact") || "Quick Contact & Appointments"}
              </h2>
              <p className="mt-2 text-slate-700">
                {t(
                  "contact.quickContactDesc",
                  "Use the quick actions below to call, WhatsApp, or view our visiting hours. For detailed enquiries, email us or visit the hospital."
                )}
              </p>
              ...
              <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  onClick={callPrimary}
                  className="w-full inline-flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 bg-white text-slate-900 shadow-sm hover:shadow-md transition"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full inline-flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 bg-[#25D366] text-white shadow-sm hover:brightness-95 transition"
                >
                  <div className="flex-shrink-0">
                    {/* Simple phone icon; replace with WA svg if preferred */}
                    <PhoneCall size={18} className="text-white" />
                  </div>
                  <div className="text-left">
                    <div className="text-sm font-medium">WhatsApp</div>
                    <div className="text-xs text-white/90">{displayPhone}</div>
                  </div>
                </a>
                ...
                <div className="w-full inline-flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 bg-white text-slate-900 shadow-sm transition">
                  <ShieldCheck size={18} className="text-emerald-600" />
                  <div className="text-left">
                    <div className="text-sm font-medium">
                      {t("contact.emergency") || "Emergency"}
                    </div>
                    <div className="text-xs text-slate-600">
                      {phones[1].number}
                    </div>
              ...
              <div className="mt-4 text-sm text-slate-600">
                <p>
                  {t(
                    "contact.note",
                    "For scheduled appointments, please call or WhatsApp us. Emergency cases should use the emergency number immediately."
                  )}
                </p>
              </div>
            </div>
      ...
      {/* 3) Contact Details (4 cards one-per-row below) */}
      <section className="relative mt-10 sm:mt-14 lg:mt-16">
